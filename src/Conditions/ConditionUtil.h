...
namespace Conditions::ConditionUtil
{
	...
	namespace Script
	{
		...
		inline float GetNumProperty(ObjectPtr a_obj, RE::BSFixedString a_prop)
		{
			auto var = a_obj->GetProperty(a_prop);

			if (var->IsBool()) {
				return (float)RE::BSScript::UnpackValue<bool>(var);
			}
			if (var->IsInt()) {
				return (float)RE::BSScript::UnpackValue<int>(var);
			}

			return RE::BSScript::UnpackValue<float>(var);
		}
	}
}